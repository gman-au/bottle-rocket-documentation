...
|`PUBLIC_BASE_URL` | Set this value if you would like to enable [QR-based quick-connect authentication](./server-installation/qr-code-authentication) using the mobile app. This value will be the 'front-facing' URL of the locally-hosted API, and will depend on your hosting configuration (reverse-proxy etc.) settings.  |
